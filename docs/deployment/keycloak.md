...
|---|---|---|
| `clients.effective.yaml` | `task keycloak:merge` | core plus this deployment's domain overlays. **No longer what gets synced** — the syncer is handed the same files directly |
...
!!! warning "Sync every file that declares the realm, never the core alone"
    ...
    clients the core also declares. The syncer recomputes each client's grants from what it is
    given — so handing it the core alone does not under-provision, it **removes** the overlay's
    grants from the live realm, silently and with no flag involved.

    Pass them all to one sync:

    ```bash
    celine-policies keycloak sync clients.yaml --overlay clients.energy.yaml
    ```
