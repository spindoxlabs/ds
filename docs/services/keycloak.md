...
| `clients.effective.yaml` | **generated**: core + overlays. No longer applied — see the note below |
...
!!! warning "Sync every file that declares the realm, never the core alone"
    The syncer **recomputes** every client's grants from what it is given. Handing it
    `clients.yaml` alone does not under-provision — it *removes* whatever the overlay
    granted, silently, with no flag involved.

    It no longer has to be one file: `celine-policies keycloak sync` takes the core and a
    repeatable `--overlay` per further declaration, and merges them before applying anything.
    That is what `keycloak-sync` passes. `clients.effective.yaml` is still generated and is
    no longer applied by anything.
...
| Task | Effect |
|---|---|
| `task keycloak:merge` | regenerate `clients.effective.yaml` from the core plus each named overlay (`OVERLAYS=energy` by default) — nothing applies its output |
...
2. **`keycloak-sync`** — applies `clients.yaml` plus each overlay, passed as
   `sync /app/clients.yaml --overlay /app/clients.energy.yaml`: creates the client scopes and
   the service clients, and attaches the audience mappers.
