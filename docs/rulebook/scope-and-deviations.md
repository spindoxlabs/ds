...
|---|---|---|
| `DSSC-DSO-12` | [Catalogue and metadata](catalogue-and-metadata.md) `C-13` | **Reopened 2026-08-09, deliberately.** A machine-readable projection of this rulebook existed and `ds-governance validate` cited rule ids from it; both were deleted the same day. Measuring the projection is what decided it: **90 of the 113 rules marked *Enforced* cited no evidence**, and a resolving citation proved only that a file exists. So *metadata checked against this rulebook* would have meant metadata checked against an unevidenced claim, with a count published beside it that read as measurement. Closing this properly means statuses backed by evidence that runs — not a parser |
