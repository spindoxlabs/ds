...
| # | Rule | Status |
|---|---|---|
| D-14 | The wildcard admits any party **inside the circle** for that controller and purpose. It never admits a new controller and never a new purpose | **Partly enforced.** *A new purpose* is refused: every reader matches the row's purpose, and an empty purpose admits nothing (`D-7`). The controller is stamped on the row from the offer, and since 2026-08-28 it is also in the `L-2` consent fingerprint — the hash tuple carried `controller_role` alone until then, so a dimension the wildcard refuses to cross was invisible in the evidence proving which consent state authorised a handover (see `L-2` in [Provenance and logging](provenance-and-logging.md)). **A new controller is not refused.** The circle (`services/connector/src/connector/services/circle.py`) is consulted only when no consent covers the request: `ConsentPendingGuard` stops at `satisfied` before it reads `should_ask`, and `/internal/consent/check` and `/internal/dataplane/authorize` both read `get_granted_subject_ids` for the requesting consumer, which includes every wildcard row. So an independent controller holding an accepted agreement for the purpose reaches everyone with an offer-scoped grant, and nobody is asked. The tests naming this rule evidence the purpose and fingerprint halves; none asserts the refusal |
