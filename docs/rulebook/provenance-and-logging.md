...
| `CatalogueWithdrawn` | a sync takes a dataset governance no longer declares off offer ([ADR-0017](../decisions/ADR-0017-the-sync-reconciles-and-says-so.md)) | control |
...
| # | Rule | Status |
|---|---|---|
| L-1 | Every participant records all **sixteen** types. Recording is not optional and not per-dataset (`PTO-40`, `-41`) | **Enforced** since 2026-08-09. All sixteen have a reachable emitter in the connector, asserted by `services/connector/tests/test_prov_bridge_emitters.py`, whose `NOT_EMITTED_BY_THIS_CONNECTOR` set is now empty. `DataDisclosed` gained `POST /admin/disclosure` — previously emitted **out of this repository** by the onboarding service, which no deployment without that service recorded, and which could not satisfy `L-2` either since the hash it requires is computable only from the connector's own consent DB. **`UsageObligationFulfilled` was deleted rather than implemented**, and the reasoning is under `L-1a` — which took the count to fifteen. **`CatalogueWithdrawn` took it back to sixteen on 2026-09-20**, and for the opposite reason: the reconcile [ADR-0017](../decisions/ADR-0017-the-sync-reconciles-and-says-so.md) shipped takes a dataset off offer, and until then it did so silently — the graph's last word on that dataset was its publication, which is a record that is wrong rather than merely short. Emitted by `_withdraw_stale` in `services/connector/src/connector/services/provider_service.py`, after EDC accepts the delete and only for a dataset governance no longer declares; a dataset republished under a new asset id is still on offer and is not withdrawn |
...
| L-2 | A `DataDisclosed` event carries a `consent_snapshot_hash` — a recomputable SHA-256 over the authorising consent tuples — proving *which* consent state backed the handover | **Enforced.** `consent_snapshot_hash` and `dataset_id` are both **required** on `DataDisclosed`, and the hash is pattern-checked as a bare lowercase SHA-256 digest — `"unknown"` and `"pending"` satisfy "the field is present" and prove nothing. The two are one requirement: a digest with no dataset id is a number nobody can recompute, and the dataset was not on the event at all. `POST /admin/disclosure` computes the hash from `consent_service.dataset_consent_snapshot` — the same function `DataIngested` uses — so the ordinary path cannot omit it, and a caller could not honestly supply one anyway. **Since 2026-08-27 that route also takes an `offer_id`**, because its caller has one and cannot have a dataset key: `D-13` keeps those out of the public projection deliberately, so an export scoped to one sharing offer had no way to name the argument the route demanded. The offer is resolved through `consent_vocabulary.datasets_for_offer` — the same authority `POST /consent/admin/shares` expands an offer with — and **one `DataDisclosed` is emitted per resolved dataset**, each with that dataset's own hash. Nothing about this rule moves: the hash stays dataset-scoped and stays recomputable. **Since 2026-08-28 it is recomputable in fact as well as in wording** — `latest_granted_rows_for_dataset` collapsed on `(subject_id, consumer_id)` and ignored `offer_id`, so a subject whose latest decision was a *decline* of one offer vanished from the snapshot along with the grant they still held on another offer over the same dataset. The hash was then **narrower than the disclosure it authorised** and depended on the order two unrelated decisions were made in, which is the one direction this rule cannot tolerate. Keyed per offer now, matching the enforcement path (`D-14`, `D-15`). That fix changed which rows are hashed and left the tuple alone. **The tuple itself changed on 2026-08-28, gaining `controller`**, which was absent: `D-11` names the consent key `(subject, purpose, controller-role)` and says matching on the controller *alone* is insufficient — making the role necessary, not the controller irrelevant — while `D-14` makes the controller decisive in as many words, since the wildcard "never admits a new controller". A dimension the wildcard refuses to cross has to be visible in the evidence, and without it two offers over one dataset agreeing on purpose and controller role but naming different controllers hashed identically. Not reachable on the offers shipped here, whose purposes separate them; reachable as soon as two controllers share a role and a purpose over one dataset, which `D-11a` does nothing to forbid. **Hashes recorded before that date were computed over the old tuple and are not migrated** — a stored digest is evidence of what it was computed over, and rewriting it to match new code destroys the property the record exists for. The tuple is still not keyed on the offer: an offer carries a consent key rather than adding a dimension to one, and a negotiated ask (`D-16`) frequently names no offer at all. Whether the hash should become offer-scoped remains a rulebook question and neither change decided it. The expansion is server-side rather than in the caller because `datasets_for_offer` returns a **list**, and a caller reading its first element is correct until a second dataset declares the same offer and then silently wrong. `services/provenance/tests/test_events_consent.py`, `services/connector/tests/test_provenance_events.py`, `ds-e2e --flow onboarding-seam` |
| L-3 | Provenance records carry codes, pseudonymous DIDs and hashes only, never PII | **Enforced** — see [Personal data](personal-data.md) D-2. **One breach measured and closed, 2026-09-20:** `QueryExecuted.authorized_subject_ids` carried raw email addresses — 22 in a single run. Not invented by the data plane: ds hands a PEP the row filter's `principals`, which are registry-native by necessity (the handler matches on them, and in this realm a username *is* an email), and the real data plane echoed that list into its audit call. `POST /internal/audit/query` now keeps only `did:` values, logging the count and the dataset and never a value. **Dropped rather than refused** — rejecting would lose the consumer, the agreement and the row count, and `services/dataset-api-mock` refuses the whole query when its audit call fails. The field is DIDs by specification, not merely by preference: `GET /audit/log?subject_id=` filters the access log on this very list, and a person asks with a DID — so an address here never matched, which made the subject's own audit query silently empty. **The record is complete again since the same day:** `DataplaneRowFilter.subject_dids` carries the consenting subjects' DIDs beside the registry-native principals, the connector fills it, and both PEPs — `services/dataset-api-mock` and the celine `dataset-api` — report it instead of the principals. The field is optional at both ends on purpose: it narrows nothing, so a decision that omits it thins the record instead of refusing the query, which is what leaves a safe rebuild order (**the data plane first, the connector second** — the reverse is a `502` on every filtered dataspace query). `services/connector/tests/test_provenance_events.py`, `services/connector/tests/test_dataplane_authorize.py`, `libs/governance/tests/test_dataplane_contract.py` |
...
| L-7 | The JSON-LD `@context` defines every relation the ingest path can produce | **Enforced, by a sweep rather than by review.** `invalidated` is defined in `PROV_CONTEXT` and accepted by `POST /prov/relations`, which used to reject the one term its own ingest path writes. **Both invalidation directions are now defined**, and they are not duplicates: `prov:invalidated` (Activity→Entity) is what `AccessRevoked` and `ConsentRevoked` write, `prov:wasInvalidatedBy` (Entity→Activity) is what `CatalogueWithdrawn` writes — and the direction decides where the fact can be found, since the lineage walk follows edges backwards in time from the thing they are about. A withdrawal on the dataset's own record sits beside the `wasGeneratedBy` its publication wrote. `services/provenance/tests/test_relation_vocabulary.py` scans the materialisers and fails on any relation the schema or the context does not also carry. `actedOnBehalfOf` and `wasInformedBy` remain accepted without a materialiser deliberately: this is a general PROV-O graph API, and the manual door should not be narrower than the vocabulary it publishes |
...
| L-13 | A service reading provenance holds a scope that permits it, and **a write scope is not one** | **Enforced, and tightened on 2026-09-20 after the opposite was measured live.** Every read route had accepted `provenance.read` **or** `.write`, on the reasoning that *"a caller trusted to write the graph is not a narrower principal than one trusted to read it"* and that a store is one participant's anyway. Both halves failed in a deployment where two participants share one realm: three organisation clients holding `provenance.write` alone read **every event in another participant's store**, a collector's withdrawal `reason` included ([ADR-0019](../decisions/ADR-0019-a-collector-says-why-it-withdrew.md)'s amendment). A store is per participant; a **realm is not**, and only the scope stood between them. Reads now require `provenance.read` and writes `provenance.write`, stated **per route** rather than at a router — a router-level floor is ANDed with the route's, so a mixed router can only ever carry the weaker of the two, which is how this arose. The widening's original beneficiary no longer exists either: `ProvenanceClient.get_lineage` was deleted ("this client writes; it does not read"). Who reads today: an operator, through the portal, under their **own** token — `ds-admin`, `ds-participant-admin` and `ds-participant-viewer` all carry `provenance.read` — and `svc-ds-e2e` in a dev or CI realm. Who writes and no longer reads: every connector's organisation client, and the out-of-repo onboarding service. A **data subject** reads their own record through `GET /prov/my/events` and is unaffected: it is scope-free by design (`L-11`). `services/provenance/tests/test_auth.py` pins all three directions, and sweeps every route so the next one cannot be added without a scope. **Rows already written are not cleaned**: what a deployment recorded before this date it still holds |
...
all sixteen event types having a reachable emitter (`L-1`).
`UsageObligationFulfilled` was **deleted** rather than implemented (`L-1a`): a consumer's
unverifiable self-report is not provenance, and neither DSP nor EDC models one. The count
returned to sixteen on 2026-09-20 with `CatalogueWithdrawn`, which is the opposite case —
a fact the platform produces and nobody records.
