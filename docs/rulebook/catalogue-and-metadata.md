...
| C-13 | Metadata is checked for compliance with this rulebook (`DSO-12`) | **Partly enforced.** The blocker is gone: something reads this document. `libs/ds-e2e/src/ds_e2e/rulebook.py` parses all nine pages into `docs/rulebook/rules.json` — every rule with its id, status and cited evidence — which the site serves. The parse is **total**: the `| # | Rule | Status |` header is what makes a table a rule table, every row beneath one must parse or the build fails, and a rule-shaped row outside one is reported — so a rule cannot go uncounted in silence, which is how the first version read 128 of 137. Then `libs/ds-e2e/tests/test_rulebook_projection.py` checks *the record itself* on every CI run: a closed status vocabulary, unique ids whose prefix matches the page that states them, every cited file still existing, and every open non-conformance in [Scope and deviations](scope-and-deviations.md) §4 still citing at least one rule that is not enforced. It found two defects the hour it was written. **What remains** is the other direction: `ds-governance validate` checks metadata against DCAT-AP (`C-12`) and against the ODRL profile, but not yet against a rule in this projection |
...
| C-21 | Offering visibility may be restricted to a subset of participants (`PUB-03`) | **Partly enforced** — expressible through ODRL membership constraints, which gate *negotiation*, not *visibility*. A restricted offering is visible in the catalogue and refuses at negotiation. Recorded as a deviation |
