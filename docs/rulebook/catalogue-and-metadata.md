...
| C-2 | The federated index is advisory. A discrepancy between index and provider is resolved in favour of the provider | **Declared.** Since `C-21` became enforced at discovery (2026-09-17) the index is also *narrower* than the union of what providers publish: the crawl carries its own participant identity, so a provider's access policy filters the catalogue it is served, and a dataset restricted to named recipients is absent unless the crawler is one of them. Accepted — see scope-and-deviations §3.2 |
...
| C-19 | A discovering consumer must be a registered participant (`PUB-27`) | **Enforced** over DSP, and on `POST /consumer/catalog` since defect **P0-1** closed. `require_consumer_catalog_caller` authenticates by **either** mechanism and never neither — a service on `connector.consumer.read`, or a natural person presenting a `ConsumerUser` VC-JWT; the `CatalogViewed` event is attributed to whichever proved itself, not to a caller-supplied header. The counterparty is checked against the registry through `GET /participants/resolve` (one active participant, no scopes disclosed), not the `/admin` listing, since 2026-09-17. `services/connector/tests/test_consumer_catalog_auth.py`, `services/connector/tests/test_registry.py`, `services/identity-registry/tests/test_participants_resolve.py` |
| C-20 | An unauthorised consumer's request must not even collect matching offerings (`PUB-32`) | **Enforced** over DSP |
| C-21 | Offering visibility may be restricted to a subset of participants (`PUB-03`) | **Enforced since 2026-09-17**, at discovery as well as at negotiation. The ContractDefinition's **access** policy carries the restriction — dataspace membership, and `odrl:recipient` with the DIDs the dataset's sharing offers name — and EDC evaluates that policy with a `CatalogPolicyContext` both when it builds a catalogue (`ContractDefinitionResolverImpl`) and when it validates an initial offer (`ContractValidationServiceImpl`). So a participant outside the set does not see the dataset and is refused if it asks by id. It was *partly* enforced and accepted as such on 2026-08-09, because one policy served both slots and the operand was bound only in the negotiation scope; `the-owner-scope-is-a-string-nobody-grants` separated the two. **The recipient restriction is opt-in per dataset** (`access_requirements: partner`): deriving it for every dataset that declares an offer would close `D-15`'s per-party path, since a negotiation the access policy refuses never reaches the consent layer that would have admitted the party. `access_level: secret` and `expose: false` remain what a producer uses to publish nothing at all |
