...
**Agent working material lives in a private store, and never here.** The store is a
directory outside this tree, found by convention rather than by a committed path, and
excluded from version control on this side.

| Content | Home |
|---|---|
| durable knowledge, playbooks, plans, execution state | the store |
...
The store's location differs per machine and is never committed. `AGENTS.md` states where
to look for it. Where it is not to hand, ask for it — nothing agent-facing is written into
...
- Working material is not in the clone. It survives in the store, which is where it is
  read from and the only source of truth for it.
