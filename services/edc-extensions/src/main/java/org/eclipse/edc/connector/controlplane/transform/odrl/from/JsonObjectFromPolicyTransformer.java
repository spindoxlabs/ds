 ...
 *   registries. `TypeTransformerRegistryImpl.forContext` is memoised, `register`
 *   appends to an ArrayList, lookup is `findAny()`, and there is no removal API —
 *   so a transformer registered by our extension is always appended after EDC's
 *   and never selected. There is no configuration seam either: `TransformerConfig`
 *   carries only `participantsAsId` and `omitEmptyRules`.
 *
 * WHAT IS FIXED
 *   `Expression.Visitor` is parameterised to `JsonObject`, so
 *   `visitLiteralExpression` cannot return an array and collapses a multi-valued
 *   right operand with `toString()`. A multi-purpose policy is therefore published
 *   as `"[{@value={valueType=STRING, chars=https://…}}, …]"` — a Java object dump
 *   that no counterparty can parse. Enforcement is unaffected (see Purposes), so
 *   the damage is visible only outside this JVM: the DSP catalogue, the federated
 *   catalogue, the portal and the DCAT-AP evidence.
 *
 * DELETING THIS
...
public class JsonObjectFromPolicyTransformer extends AbstractJsonLdTransformer<Policy, JsonObject> {
    ...
    /**
     * Proves which copy of this class ended up in the shadow JAR.
     *
     * <p>Two classes with this name reach the JAR — upstream's and this fork — and the
     * classloader takes whichever the packaging wrote first. `:edc-connector:verifyForkedTransformer`
     * greps the packaged class for this constant and fails the build if the upstream
     * copy won, so the fix cannot be silently lost to a change in dependency order.
     */
    public static final String FORK_MARKER = "ds-fork-multivalued-right-operand";
    ...
    private static class Visitor implements Policy.Visitor<JsonObject>, Rule.Visitor<JsonObject>, Constraint.Visitor<JsonObject>, Expression.Visitor<JsonObject> {
        ...
        public JsonObject visitAtomicConstraint(AtomicConstraint atomicConstraint) {
            ...
            constraintBuilder.add(ODRL_RIGHT_OPERAND_ATTRIBUTE, rightOperand(atomicConstraint.getRightExpression()));

            return constraintBuilder.build();
        }

        /**
         * Keys a literal can hide behind once JSON-P types have not survived a store
         * round trip: Jackson rehydrates a {@code JsonString} as its bean properties,
         * so the value sits one level deeper than the JSON-LD shape suggests.
         */
        private static final List<String> LITERAL_KEYS = List.of(VALUE, "string", "chars");

        /**
         * Renders a right operand, preserving its multiplicity.
         *
         * <p>Symmetric with {@code JsonObjectToConstraintTransformer}, which documents
         * that "Json-Ld arrays that contain multiple elements will be returned as-is".
         * Single-valued operands are unaffected.
         */
        private JsonValue rightOperand(Expression expression) {
            if (expression instanceof LiteralExpression literal) {
                ...
                if (value instanceof Collection<?> values) {
                    return values.stream().map(this::literalNode).collect(toJsonArray());
                }
            }
            return expression.accept(this);
        }

        /** One element of a multi-valued operand, as an expanded JSON-LD node. */
        private JsonValue literalNode(Object value) {
            if (value instanceof JsonValue jsonValue) {
                return jsonValue;
            }
            if (value instanceof Map<?, ?> map) {
                var id = map.get(ID);
                if (id != null) {
                    return jsonFactory.createObjectBuilder().add(ID, String.valueOf(id)).build();
                }
                for (var key : LITERAL_KEYS) {
                    var nested = map.get(key);
                    if (nested != null) {
                        return literalNode(nested);
                    }
                }
            }
            return jsonFactory.createObjectBuilder()
                    .add(VALUE, Json.createValue(String.valueOf(value)))
                    .build();
        }
