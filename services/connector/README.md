...
- `POST /consent/admin/shares` — a service (onboarding) provisions a subject's standing consent from an `offer_id`; guarded by `connector.consent.provision`. Writes `consumer_id = "*"` wildcard rows with a non-PII `legal_basis` record. **Granting requires evidence** — `source`, `consent_text_version` and `rendered_text_sha256`, else 422; withdrawal requires none. An organisation's own withdrawal (`decided_by: "collector"`) may say why in `reason` (≤ 200 characters), recorded on the row and in provenance and returned by no read (`D-12a`). See [this service's page](https://spindoxlabs.github.io/ds/services/connector/)
