...
class TestOwnerCRUD:
    ...
    async def test_create_duplicate_owner(self, client, admin_headers):
        await client.post("/admin/owners", json=EXAMPLE_OWNER, headers=admin_headers)
        resp = await client.post(
            "/admin/owners", json=EXAMPLE_OWNER, headers=admin_headers
        )
    ...
    async def test_get_owner(self, client, admin_headers):
        await client.post("/admin/owners", json=EXAMPLE_OWNER, headers=admin_headers)
        resp = await client.get("/admin/owners/example-org", headers=admin_headers)
        assert resp.status_code == 200
        assert resp.json()["id"] == "example-org"

    @pytest.mark.asyncio
    async def test_get_owner_not_found(self, client, admin_headers):
        resp = await client.get("/admin/owners/nonexistent", headers=admin_headers)
    ...
    async def test_delete_owner(self, client, admin_headers):
        await client.post("/admin/owners", json=EXAMPLE_OWNER, headers=admin_headers)
        resp = await client.delete("/admin/owners/example-org", headers=admin_headers)
    ...
    async def test_delete_owner_not_found(self, client, admin_headers):
        resp = await client.delete("/admin/owners/nonexistent", headers=admin_headers)
        assert resp.status_code == 404
...
class TestOwnerResolve:
    ...
    @pytest.mark.asyncio
    async def test_resolve_canonical_uri_url_fallback(
        self, client, admin_headers, read_headers
    ):
        owner = {
...
class TestOwnerAuth:
    ...
    async def test_resolve_requires_read_scope(self, client):
        resp = await client.get("/owners/resolve", params={"alias": "test"})
