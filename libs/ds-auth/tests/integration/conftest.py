...
the sync was never run — or ran against a stale `clients.effective.yaml` — and
...
def declared_clients() -> list[dict]:
    ...
    Read from `clients.yaml`, **not** `clients.effective.yaml`: the effective
    file is generated by merging a deployment's domain overlay, and what this
    suite asserts is the contract *ds* declares. A realm granting more than ds
    asks for is a deployment's business; granting less is the defect.
    """
