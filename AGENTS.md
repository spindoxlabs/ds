...
| **What is done, and what is missing?** | **`docs/rulebook/status.md`** — generated by `task rulebook:status` from the blueprints, the rulebook and the markers on the tests. Regenerate it, do not reason about it |
...
`task rulebook:status` collects them, so a rule's enforcement status is *derived* from the
tests that exist rather than asserted in prose — see
[docs/development/conformance.md](docs/development/conformance.md). A rule reading
**Enforced** that no test names is reported, and that report is the point: the previous
hand-written projection had 79% of its *Enforced* rules citing no evidence at all.
...
`compliance:*`, `rulebook:*`, `docs:*`, plus the `docker:*` / `dev:*` lifecycle pairs. Per-unit Taskfiles
